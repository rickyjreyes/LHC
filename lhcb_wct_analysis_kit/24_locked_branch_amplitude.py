...
"""
24_locked_branch_amplitude_ladder.py

Locked-Branch Cross-Region Amplitude-Cap Ladder
------------------------------------------------

Purpose:
    Repeat the locked-winding cross-region test across amplitude caps.

Question:
    Are branch rankings stable when A_MAX is lowered?

Caps:
    A_MAX = 0.10, 0.075, 0.05, 0.025

Decision:
    If branch rankings persist while bound_active decreases:
        branch preference is more meaningful.

    If rankings change strongly:
        previous ranking was partly amplitude-cap allocation.

    If all fits remain bound-active even at low caps:
        residual structure is stronger than the bounded linear branch model.
"""
...
OUTDIR = "outputs_wct_locked_branch_amplitude_ladder"
...
A_MAX_GRID = [0.10, 0.075, 0.05, 0.025]
...
def fit_poisson_bounded(counts, baseline, ell, ks, a_max):
    ...
    coef_bound_active = any(abs(v) >= a_max - 1e-5 for v in beta[1:])

    # This detects polar-amplitude saturation caused by both cos/sin components
    # hitting bounds, where amp can reach sqrt(2)*a_max.
    amp_bound_active = any(x >= math.sqrt(2.0) * a_max - 1e-5 for x in amps)
    ...
    return {
        ...
        "amps": amps,
        "phases": phases,
        "coef_bound_active": bool(coef_bound_active),
        "amp_bound_active": bool(amp_bound_active),
        "bound_active": bool(coef_bound_active or amp_bound_active),
        "n_params": int(p),
    }
...
class BranchResult:
    ...
    max_amp: float
    phases: str
    coef_bound_active: bool
    amp_bound_active: bool
    bound_active: bool
    success_base: bool
    success_branch: bool
    branch_rank_by_deltaD: int = -1
...
def run():
    ...
    for region_cfg in REGIONS:
        ...
        for a_max in A_MAX_GRID:
            print("\n" + "-" * 100)
            print(f"[cap] A_MAX={a_max}")
            print("-" * 100)

            base_fit = fit_poisson_bounded(counts, baseline, ell, ks=[], a_max=a_max)
            D_base = base_fit["dev"]
            ...
            for branch in BRANCHES:
                ...
                fit = fit_poisson_bounded(counts, baseline, ell, ks=k_vals, a_max=a_max)
                D_branch = fit["dev"]
                ...
                for j in range(NULL_N):
                    ...
                    base_null = fit_poisson_bounded(y_null, baseline, ell, ks=[], a_max=a_max)
                    branch_null = fit_poisson_bounded(y_null, baseline, ell, ks=k_vals, a_max=a_max)
                    null_deltas.append(base_null["dev"] - branch_null["dev"])

                    if (j + 1) % 500 == 0:
                        print(
                            f"  null {j+1}/{NULL_N} "
                            f"{region_name} A={a_max} {branch['label']}"
                        )
                ...
                row = BranchResult(
                    ...
                    deltaD=float(deltaD),
                    deltaD_per_event=float(deltaD / max(n_events, 1)),
                    ...
                    max_amp=float(max(fit["amps"]) if fit["amps"] else 0.0),
                    phases=json.dumps(fit["phases"]),
                    coef_bound_active=bool(fit["coef_bound_active"]),
                    amp_bound_active=bool(fit["amp_bound_active"]),
                ...
                print(
                    f"[{branch['label']}] "
                    f"DeltaD={deltaD:.4f}, per_event={deltaD / max(n_events, 1):.6g}, "
                    f"p={p_fixed:.6g}, max_amp={row.max_amp:.6g}, "
                    f"bound={fit['bound_active']}"
                )

            # Rank within region/cap by DeltaD.
            cap_rows_sorted = sorted(cap_rows, key=lambda r: r.deltaD, reverse=True)
            for rank, row in enumerate(cap_rows_sorted, start=1):
                row.branch_rank_by_deltaD = rank

            all_rows.extend(cap_rows_sorted)

            best = cap_rows_sorted[0]
            n_bound = sum(1 for r in cap_rows_sorted if r.bound_active)

            region_cap_summary.append({
                "A_MAX": float(a_max),
                "region": region_name,
                "N_events": n_events,
                "D_base": float(D_base),
                "best_branch": best.branch_label,
                "best_deltaD": best.deltaD,
                "best_deltaD_per_event": best.deltaD_per_event,
                "best_p": best.p_fixed_branch,
                "n_bound_active_branches": int(n_bound),
                "n_total_branches": int(len(cap_rows_sorted)),
                "all_bound_active": bool(n_bound == len(cap_rows_sorted)),
            })

    result_df = pd.DataFrame([asdict(r) for r in all_rows])
    summary_df = pd.DataFrame(region_cap_summary)

    result_csv = os.path.join(OUTDIR, "locked_branch_amplitude_ladder_comparison.csv")
    summary_csv = os.path.join(OUTDIR, "locked_branch_amplitude_ladder_summary.csv")
    summary_json = os.path.join(OUTDIR, "locked_branch_amplitude_ladder_summary.json")

    result_df.to_csv(result_csv, index=False)
    summary_df.to_csv(summary_csv, index=False)

    # Pivot rank table for fast reading.
    rank_table = result_df.pivot_table(
        index=["A_MAX", "region"],
        columns="branch_label",
        values="branch_rank_by_deltaD",
        aggfunc="first",
    ).reset_index()

    rank_csv = os.path.join(OUTDIR, "locked_branch_rank_table.csv")
    rank_table.to_csv(rank_csv, index=False)

    # Stability summary.
    stability = {}
    for region, g in summary_df.groupby("region"):
        stability[region] = {
            "best_branch_sequence": [
                {
                    "A_MAX": float(r["A_MAX"]),
                    "best_branch": str(r["best_branch"]),
                    "best_deltaD": float(r["best_deltaD"]),
                    "all_bound_active": bool(r["all_bound_active"]),
                    "n_bound_active_branches": int(r["n_bound_active_branches"]),
                }
                for _, r in g.sort_values("A_MAX", ascending=False).iterrows()
            ],
            "unique_best_branches": sorted(g["best_branch"].unique().tolist()),
            "all_caps_same_best": bool(len(g["best_branch"].unique()) == 1),
            "all_caps_all_bound_active": bool(g["all_bound_active"].all()),
        }

    payload = {
        "test": "locked_branch_amplitude_cap_ladder",
        "purpose": "Check whether locked-branch rankings persist as amplitude cap is lowered.",
        ...
        "region_cap_summary": region_cap_summary,
        "stability": stability,
        "files": {
            "comparison_csv": result_csv,
            "summary_csv": summary_csv,
            "rank_table_csv": rank_csv,
            "summary_json": summary_json,
        },
        "interpretation": {
            "rankings_persist_and_not_bound": "Branch preference is more interpretable.",
            "rankings_persist_but_bound": "Strong structure, but amplitude cap still controls absolute deviance.",
            "rankings_change": "Previous branch ranking may be cap-sensitive.",
            "all_bound_active_at_low_caps": "Bounded linear branch model underfits the residual; consider regularized amplitudes.",
        },
    }
    ...
    print("\n" + "=" * 100)
    print("AMPLITUDE LADDER SUMMARY")
    print("=" * 100)
